...
Libtest(){
	...
	if [ "$libAddr" == "$GIT" ];then
		libAddr='https://raw.githubusercontent.com/biux0x/ss-panel-v3-mod-node-connect/master/libsodium-1.0.13.tar.gz'
	else
...
install_centos_ssr(){
	...
	ldconfig
	git clone -b manyuser https://github.com/biux0x/shadowsocks.git "/root/shadowsocks"
	cd /root/shadowsocks
...
install_ubuntu_ssr(){
	...
	pip install cymysql
	cd /root
	git clone -b master https://github.com/biux0x/shadowsocks.git "/root/shadowsocks"
	cd shadowsocks
...
install_node(){
	...
	wget -N -P  /etc/ --no-check-certificate  https://raw.githubusercontent.com/biux0x/ss-panel-v3-mod-node-connect/master/supervisord.conf
	supervisord
	#iptables
	iptables -F
	iptables -X  
